    ...
    : AudioProcessorEditor (&p), audioProcessor (p)
{
    ...
    webConfig.pluginDisplayName = "Zenith";
...
void ZenithAudioProcessorEditor::timerCallback()
{
    int noteNumber = 0;
    int velocity = 0;
    while (audioProcessor.consumeLatestMidiNote (noteNumber, velocity))
        notifyMIDINote (noteNumber, velocity);

    if (isAuthorized)
    {
        static int authRecheckCounter = 0;
        if (++authRecheckCounter >= 30000)
        {
            authRecheckCounter = 0;
            isAuthorized = checkAuthorization();
        }
        return;
    }

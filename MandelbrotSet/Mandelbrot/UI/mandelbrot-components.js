/**
 * @fileoverview Mandelbrot Components Library - Reusable UI Components
 * @module Mandelbrot Components
 * 
 * Provides interactive control components for the Mandelbrot plugin line:
...
class MandelbrotKnob {
  ...
  init() {
    ...
    this.element.addEventListener('mouseenter', () => {
      this.element.style.animation = 'breathe 2s ease-in-out infinite';
    });
  ...
  onWheel(e) {
    ...
    // Add subtle scale animation on wheel
  ...
  setValue(newValue) {
    ...
    // Add ripple effect on significant changes
    if (Math.abs(this.value - oldValue) > (this.options.max - this.options.min) * 0.05) {
      this.addRipple();
    }
  ...
  addRipple() {
    ...
    ripple.style.border = '2px solid var(--plugin-primary)';
...
class MandelbrotSlider {
  ...
  init() {
    ...
    // Add hover effects
    this.element.addEventListener('mouseenter', () => {
      this.element.style.transform = 'scaleY(1.1)';
    });
    this.element.addEventListener('mouseleave', () => {
      if (!this.isDragging) {
        this.element.style.transform = '';
  ...
  onWheel(e) {
    ...
    // Add subtle pulse on wheel
    this.element.style.transform = 'scaleY(1.15)';
    setTimeout(() => {
      this.element.style.transform = '';
    }, 100);
...
class MandelbrotTooltip {
  ...
  init() {
    ...
    this.tooltip.innerHTML = `
      <div class="mandelbrot-tooltip-title"></div>
      <div class="mandelbrot-tooltip-description"></div>
    `;
  ...
  show(element, title, description) {
    ...
    // Update content
    this.tooltip.querySelector('.mandelbrot-tooltip-title').textContent = title;
    this.tooltip.querySelector('.mandelbrot-tooltip-description').textContent = description;
...
const mandelbrotTooltip = new MandelbrotTooltip();

// Auto-register on DOM ready
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
      mandelbrotTooltip.autoRegister();
      setupHelpToggle();
    }, 100);
  });
} else {
...
function setupHelpToggle() {
  ...
  if (helpButton) {
    helpButton.addEventListener('click', () => {
      const isEnabled = mandelbrotTooltip.toggle();
...
if (typeof window !== 'undefined') {
  window.MandelbrotKnob = MandelbrotKnob;
  window.MandelbrotSlider = MandelbrotSlider;
  window.MandelbrotXYPad = MandelbrotXYPad;
  window.MandelbrotRangeSlider = MandelbrotRangeSlider;
  window.MandelbrotTooltip = MandelbrotTooltip;
  window.mandelbrotTooltip = mandelbrotTooltip;

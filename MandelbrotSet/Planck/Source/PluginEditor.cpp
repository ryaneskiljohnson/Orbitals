...
//==============================================================================
PlanckAudioProcessorEditor::PlanckAudioProcessorEditor (PlanckAudioProcessor& p)
...
PlanckAudioProcessorEditor::~PlanckAudioProcessorEditor()
{
}

void PlanckAudioProcessorEditor::paint (juce::Graphics& g)
{
    g.fillAll (juce::Colour (0xff12121a));
}

void PlanckAudioProcessorEditor::resized()
{
    if (webView != nullptr)
        webView->setBounds (getLocalBounds());
}

void PlanckAudioProcessorEditor::loadWebUI()
{
    auto htmlContent = MandelbrotEditorHelpers::loadPluginHTML("Planck");
    if (htmlContent.isNotEmpty())
    {
            // Load HTML using temporary file approach (avoids data URL encoding issues)
    auto tempDir = juce::File::getSpecialLocation(juce::File::tempDirectory)
        .getChildFile("PlanckUI_" + juce::String(juce::Time::currentTimeMillis()));
...
void PlanckAudioProcessorEditor::loadHTMLFile (const juce::File& htmlFile)
{
    // Not used - using helper instead
}

void PlanckAudioProcessorEditor::handleJavaScriptMessage (const juce::var& message)
{
    if (!message.isObject())
        return;
    ...
    if (type == "parameterChange")
    {
        ...
        if (param == "lowfreq" || param == "lowgain" || param == "midfreq" || param == "midgain" || param == "highfreq" || param == "highgain" || param == "q")
        {
        ...
        {
            auto* p = audioProcessor.parameters.getParameter(PlanckAudioProcessor::PARAM_BYPASS);
            if (p != nullptr)
...
//==============================================================================
void PlanckAudioProcessorEditor::loadAuthScreen()
{
...
<head>
    ...
    <title>Planck - Authentication Required</title>
    <style>
...
<body>
    ...
    auto projectRoot = juce::File("/Users/rjmacbookpro/Development/Orbitals/MandelbrotSet");
    auto backgroundImage = projectRoot.getChildFile("_Shared/Assets/backgrounds/planck.png");
    ...
    auto tempDir = juce::File::getSpecialLocation(juce::File::tempDirectory)
        .getChildFile("PlanckAuth_" + juce::String(juce::Time::currentTimeMillis()));
    tempDir.createDirectory();
...
//==============================================================================
bool PlanckAudioProcessorEditor::checkAuthorization()
{
...
//==============================================================================
juce::File PlanckAudioProcessorEditor::getAuthFile()
{
...
//==============================================================================
juce::String PlanckAudioProcessorEditor::loadAndDecryptLicenseFile()
{
...
//==============================================================================
void PlanckAudioProcessorEditor::timerCallback()
{
...
void PlanckAudioProcessorEditor::sendMeteringData()
{

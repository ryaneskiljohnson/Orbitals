...
{
    DBG("=== PixelPulse Editor Constructor ===");
...
void PixelPulseAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    if (webView == nullptr || !webView->isVisible())
    {
        ...
        g.setFont(14.0f);
        g.drawText("Click here to focus for keyboard input", getLocalBounds().reduced(20), juce::Justification::topLeft);
        
        if (hasKeyboardFocus(true))
        {
            g.setColour(juce::Colours::green);
            g.drawText("HAS KEYBOARD FOCUS", getLocalBounds().reduced(20), juce::Justification::topRight);
        }
        else
        {
            g.setColour(juce::Colours::red);
            g.drawText("NO KEYBOARD FOCUS - CLICK TO FOCUS", getLocalBounds().reduced(20), juce::Justification::topRight);
        }
    }
}
...
void PixelPulseAudioProcessorEditor::mouseDown(const juce::MouseEvent& e)
{
...
//==============================================================================
juce::File PixelPulseAudioProcessorEditor::getAuthFile()
{
...
//==============================================================================
juce::String PixelPulseAudioProcessorEditor::loadAndDecryptLicenseFile()
{
...
//==============================================================================
void PixelPulseAudioProcessorEditor::timerCallback()
{
    if (!isAuthorized)
    {
...
void PixelPulseAudioProcessorEditor::sendMeteringData()
{
...
bool PixelPulseAudioProcessorEditor::keyPressed(const juce::KeyPress& key)
{
    DBG("=== KEY PRESSED IN C++ ===");
    ...
    if (webView != nullptr)
    {
        ...
        juce::String keyCode;
        int keyCodeInt = key.getKeyCode();
        
        DBG("Checking key code: " + juce::String(keyCodeInt) + " against known keys");
        
        if (keyCodeInt == 'A' || keyCodeInt == 'a') keyCode = "KeyA";
        else if (keyCodeInt == 'S' || keyCodeInt == 's') keyCode = "KeyS";
        else if (keyCodeInt == 'D' || keyCodeInt == 'd') keyCode = "KeyD";
        else if (keyCodeInt == 'F' || keyCodeInt == 'f') keyCode = "KeyF";
        else if (keyCodeInt == 'J' || keyCodeInt == 'j') keyCode = "KeyJ";
        else if (keyCodeInt == 'K' || keyCodeInt == 'k') keyCode = "KeyK";
        else if (keyCodeInt == 'L' || keyCodeInt == 'l') keyCode = "KeyL";
        else if (keyCodeInt == ';') keyCode = "Semicolon";
        ...
        {
            DBG("Key not mapped to game control: " + juce::String(keyCodeInt));
        }
    }
    else
    {

    ...
    : AudioProcessorEditor (&p), audioProcessor (p)
{
    ...
    webConfig.pluginDisplayName = "Lagrange";
...
auto type = obj->getProperty("type").toString();
        ...
        if (type == "parameterChange")
    {
        ...
        if (param == "drift" && value.isObject())
        {
            if (auto* drift = value.getDynamicObject())
            {
                setRawParameter (LagrangeAudioProcessor::PARAM_DRIFT_MIN,
                                 static_cast<float> (drift->getProperty ("min")));
                setRawParameter (LagrangeAudioProcessor::PARAM_DRIFT_MAX,
                                 static_cast<float> (drift->getProperty ("max")));
            }
            return;
        }

        if (param == "microChaos" && value.isObject())
        {
            if (auto* chaos = value.getDynamicObject())
            {
                setRawParameter (LagrangeAudioProcessor::PARAM_CHAOS_X,
                                 static_cast<float> (chaos->getProperty ("x")));
                setRawParameter (LagrangeAudioProcessor::PARAM_CHAOS_Y,
                                 static_cast<float> (chaos->getProperty ("y")));
            }
            return;
        }
...
//==============================================================================
juce::File LagrangeAudioProcessorEditor::getAuthFile()
{
...
//==============================================================================
juce::String LagrangeAudioProcessorEditor::loadAndDecryptLicenseFile()
{

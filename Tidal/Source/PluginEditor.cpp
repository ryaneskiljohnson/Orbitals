    ...
    : AudioProcessorEditor (&p), audioProcessor (p)
{
    ...
    webConfig.pluginDisplayName = "Tidal";
...
auto type = obj->getProperty("type").toString();
        ...
        if (type == "parameterChange")
    {
        ...
        if (param == "rate")
        {
            const juce::String tempo = value.toString();
            int rateIndex = 1;

            if (tempo == "1/4")       rateIndex = 0;
            else if (tempo == "1/8")  rateIndex = 1;
            else if (tempo == "1/16") rateIndex = 2;
            else if (tempo == "1/32") rateIndex = 3;

            setRawParameter (TidalAudioProcessor::PARAM_RATE, static_cast<float> (rateIndex));
            return;
        }

        if (param == "shape")
        {
            const juce::String shape = value.toString().toLowerCase();
            int shapeIndex = 0;

            if (shape == "sine")           shapeIndex = 0;
            else if (shape == "triangle")  shapeIndex = 1;
            else if (shape == "saw")       shapeIndex = 2;
            else if (shape == "square")    shapeIndex = 3;

            setRawParameter (TidalAudioProcessor::PARAM_SHAPE, static_cast<float> (shapeIndex));
            return;
        }
